...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        roomManager.addRoom(new BasicRoom(1, "Lobby", Optional.of(2), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.<Integer>absent(),
                "This is the lobby. It's pretty empty and the paint still smells fresh.\r\n"));
        roomManager.addRoom(new BasicRoom(2, "Hallway", Optional.of(3), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.of(1), Optional.<Integer>absent(), Optional.<Integer>absent(),
                "This is the hallway. It's long and hallway-ish with exposed wires and floorboards showing.\r\n"));
        roomManager.addRoom(new BasicRoom(3, "Intake", Optional.<Integer>absent(), Optional.of(6), Optional.<Integer>absent(), Optional.of(2),Optional.<Integer>absent(), Optional.<Integer>absent(),
                "This is the intake area.  People are lined up like cattle waiting to be prodded.\r\n"));
        roomManager.addRoom(new BasicRoom(6, "Closet", Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.of(3), Optional.of(7), Optional.<Integer>absent(), Optional.<Integer>absent(),
                "You find yourself in the janitorial closet.  It smells like bleach.\r\n"));
        roomManager.addRoom(new BasicRoom(7, "Toilet", Optional.of(6), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.of(8), Optional.<Integer>absent(),
                "You find yourself in the toilet. The smell is horrible. There's an opening in the ceiling.\r\n"));
        roomManager.addRoom(new BasicRoom(8, "Toilet Ceiling Crawl Space", Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.<Integer>absent(), Optional.<Integer>absent(),Optional.<Integer>absent(), Optional.of(7),
        ...
        entityManager.addEntity(new Derper(gameManager, 8));
        Iterator<Map.Entry<Integer, Room>> rooms = roomManager.getRooms();
        while (rooms.hasNext()) {
            Map.Entry<Integer, Room> next = rooms.next();
            entityManager.addEntity(next.getValue());
        }
