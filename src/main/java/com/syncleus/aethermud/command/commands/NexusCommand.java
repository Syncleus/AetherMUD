...
public class NexusCommand extends Command {
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
        execCommand(ctx, e, () -> {
            if (gameManager.getAetherMudConfiguration().isIrcEnabled) {
                originalMessageParts.remove(0);
                String transferPhrase = Joiner.on(" ").join(originalMessageParts);
                if (gameManager.getIrcBotService().getBot().isConnected()) {
                    gameManager.getIrcBotService().getBot().getUserChannelDao().getChannel(gameManager.getAetherMudConfiguration().ircChannel).send().message(player.getPlayerName() + ": " + transferPhrase);
                }
            }
        });
    }
